...
def _parse_merged_args():
    ...
    collector_parameters = [
        ...
        {"name": "--save_latent", "action": "store_true", "default": False},
        {"name": "--no_disturb", "action": "store_true", "default": False},
    ]
...
class TrajectoryDataCollector:
    def __init__(
        ...
        self.no_disturb = getattr(self.args, "no_disturb", False)
        ...
        self.save_latent = getattr(self.args, "save_latent", False)
        print("=" * 100)
        print(self.total_obs_dim, self.total_privileged_dim, "save latent:", self.save_latent)
        print("=" * 100)
        ...
        if args.const_prob > 0 and os.path.exists(self.constant_rb_path):
            if getattr(self.args, "overwrite", False):
                print(f"[Collector] Removing existing directory: {self.constant_rb_path}")
                shutil.rmtree(self.constant_rb_path)
            else:
                print(f"[Collector] Directory exists: {self.constant_rb_path} (appending)")
        if args.switch_prob > 0 and os.path.exists(self.switch_rb_path):
            if getattr(self.args, "overwrite", False):
                print(f"[Collector] Removing existing directory: {self.switch_rb_path}")
                shutil.rmtree(self.switch_rb_path)
            else:
                print(f"[Collector] Directory exists: {self.switch_rb_path} (appending)")
    ...
    def _collect_episode(self, trajectory_type: str) -> Tuple[List[Dict[str, Any]], List[float]]:
        ...
        self.env.use_disturb = not self.no_disturb
